        ...
        <MainComponent className=' max-w-2xl h-fit mt-2'>
            <div className='flex gap-1 text-xl '>
                {["#000000", "#FF0000", "#06F906", "#0000FF", "#FFFF00", "#FF00FF", "#00FFFF"].map((color, index) => (
                    <div key={index} className='p-1 hover:border hover:border-gray-300  cursor-pointer rounded-lg  w-10 h-8'>
                        <div className={`w-full h-full rounded-3xl `} style={{ backgroundColor: color }}></div>
                    </div>
                ))}
                <div className='p-1 hover:border hover:border-gray-300  cursor-default rounded-lg  w-10 h-8'>
